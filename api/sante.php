...
if($region=='*')
     $sql = "select name,type,st_asgeojson(sante.geom) as geojson from sante,region_sn where st_within(sante.geom,region_sn.geom) and nomreg='$region'";

else {
}
	$sql ="select st_asgeojson(geom) as geojson,name,type from sante";
$rs = $conn->query($sql);
if (!$rs) {
    echo 'Erreur SQL.\n';
    exit;
